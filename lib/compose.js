...
class Compose {
  ...
  stripConstraints(dependencies) {
    console.log(JSON.stringify(dependencies))
    const services = mapValues(omit('placement.constraints'), dependencies.services)
    return defaults(dependencies, { services })
  }

  toObject({ stripConstraints = false } = {}) {
    ...
    if (stripConstraints) return this.stripConstraints(object)

    return object
  }

  toYAMLFileSync(filename, { stripConstraints = false } = {}) {
    yaml.writeSync(filename, this.toObject({ stripConstraints }))
  }
}
