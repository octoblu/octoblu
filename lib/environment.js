...
class Environment {
  constructor({ services, values, templatesDir }) {
    ...
    if (isEmpty(templatesDir))
      throw new Error("Environment requires templatesDir")
  ...
  toJSON() {
    const readServiceEnv = service =>
      fs.readJSONSync(path.join(this.templatesDir, service, "environment.json"))
  ...
  _detemplateService(serviceName) {
    ...
    const envVars = fs.readJSONSync(
      path.join(this.templatesDir, serviceName, "environment.json"),
    )
    const config = {}
    each(envVars, (value, key) => {
